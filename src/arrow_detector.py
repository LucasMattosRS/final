...
class ArrowDetector:
    ...
    HOUGH_THRESHOLD = 40
    HOUGH_MIN_LEN   = 15
    HOUGH_MAX_GAP   = 30   # gap maior para capturar tracejados
    MERGE_DISTANCE  = 18
    MERGE_ANGLE_DEG = 15
    ...
    def _preprocess(self) -> np.ndarray:
        """Fecha gaps de tracejados antes do Canny."""
        ...
        closed = cv2.bitwise_or(
            cv2.dilate(binary, kh, iterations=1),
            cv2.dilate(binary, kv, iterations=1),
        )
        return cv2.Canny(closed, self.CANNY_LOW, self.CANNY_HIGH)

    # ── Problema 3 corrigido ──────────────────────────────────────────────────
    # Versao anterior: retornava `segments` ANTES de chamar _merge_segments,
    # entao a fusao de tracejados nunca era executada.
    # Correcao: o return agora e DEPOIS do merge.
    ...
    def _detect_lines(self) -> list[dict]:
        ...
        # BUG CORRIGIDO: antes havia um "return segments" aqui,
        # fazendo o merge abaixo nunca ser executado.
        return self._merge_segments(segments)   # <- agora retorna APOS o merge
    ...
    def _merge_segments(self, segments: list[dict]) -> list[dict]:
        """Une segmentos colineares proximos (reconstroi setas tracejadas)."""
    ...
    def line_destination(
        ...
        return (line["x2"], line["y2"]) if d1 < d2 else (line["x1"], line["y1"])
